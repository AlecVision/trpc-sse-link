...
export async function resolveSSERequest<
  TRouter extends AnyRouter,
  TRequest extends HTTPRequest
>(opts: ResolveSSERequestOptions<TRouter, TRequest>): Promise<HTTPResponse> {
  const {
    router,
    req,
    createContext,
  ...
  // TODO: support batching? - would this require all responses to be streamed?

  try {
    if (error) throw error;
    if (isBatchCall) toss(`Batching SSE requests is not supported`, -32022);
